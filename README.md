...
- **The subagent guard needs bash.** macOS and Linux have it, and on Windows it comes with Git for Windows (Git Bash), which Claude Code normally uses. Without bash the guard does not run. It also cannot ask you in bypass-permissions mode or in non-interactive runs (`claude -p`); there, an approval request blocks the subagent instead.
...
Plugin para Claude Code que, al iniciar cada chat, recomienda el modelo más barato (Haiku, Sonnet, Opus o Fable) y el nivel de esfuerzo adecuados para tu tarea. También muestra tu uso del límite de 5 horas, el semanal y el tamaño del contexto. Si Claude quiere lanzar un subagente con un modelo más caro que el de tu sesión, primero te pide autorización; con el mismo modelo o uno más barato lo hace sin preguntar. Se instala con los dos comandos de arriba, o pidiéndoselo a Claude, y responde en tu idioma.
